...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  constructor(mediaElement, dependencyInjector) {
    ...
    /** @private {?shaka.SeekableRange} */
    this.seekrange_ = null;
  ...
  onParseManifest_(has, wants) {
    ...
    this.regionTimeline_ =
        new shaka.media.RegionTimeline(() => this.seekRange(true));
  ...
  getBufferFullness() {
    if (this.video_) {
      ...
      const lengthToBeBuffered = Math.min(this.video_.currentTime +
          bufferingGoal, this.seekRange(true).end);
  ...
  /**
   * Set/override the range of time (in seconds) that seeking is allowed.
   * @param {shaka.SeekableRange} range
   * @export
   */
  setSeekRange(range) {
    if (range) {
      this.seekrange_ = range;
    }
  }
   ...
   */
  seekRange(real) {
    if (this.seekrange_ && !real) {
      return this.seekrange_;
    }
    if (this.manifest_) {
 ...
 * @export
 */
shaka.Player.version = 'v3.1.0-uncompiled';
