  ...
  static parse(uri, request, requestType, progressUpdated) {
    ...
    const promise = new Promise(((resolve, reject) => {
      ...
      xhr.onload = (event) => {
        ...
        try {
          // fix low latency mode working correctly:
          const responseData = shaka.util.BufferUtils.toUint8(target.response);
          const response = shaka.net.HttpPluginUtils.makeResponse(headers,
              responseData, target.status, uri, target.responseURL,
              requestType);

          if (request.streamDataCallback) {
            request.streamDataCallback(responseData).then(() => {
              resolve(response);
            }).catch(() => {});
          } else {
            resolve(response);
